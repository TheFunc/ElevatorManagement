    ...
    <form action="{{ route('file.upload') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <select name="type" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary outline-none">
                ...
                <option value="prepare" {{ old('type') == 'prepare' ? 'selected' : '' }}>准用资料</option>
        ...
        <div>
            ...
            <input type="datetime-local" name="event_time" value="{{ old('event_time') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary outline-none">
